...
def startRPC(self, port, eventListenerPort):
  ...
  logging .debug(str(os.getcwd()) + "/rpc-agent-bins/" + rpcAgentName + "-port " + str(port) + " -logfile wpwithin.log -loglevel debug,warn,error,fatal,info" + str(callbackPortString))
  process = launcherLocal.launch(cfg, os.getcwd() + "/rpc-agent-bins/" + rpcAgentName, "-port " + str(port) + " -logfile wpwithin.log -loglevel debug,warn,error,fatal,info" + str(callbackPortString))
  return process
